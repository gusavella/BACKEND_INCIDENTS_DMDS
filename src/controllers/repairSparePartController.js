...
const controller = {
    all: async (req, res) => {
      try{
        let repairSpareParts= await db.RepairSparePart.findAll({include:["repair","spare_part"]})
        res.json(repairSpareParts);
      }
      catch(e){
        console.log(e)
      }
    ...
    detail: async (req, res) => {
          ...
          try{
            let sparePart= await db.SparePart.findByPk(req.params.id)
            res.json(sparePart);
          }
          catch(e){
            console.log(e)
